...
void borders_initialize(double** board, int m, int n)
{
    ...
    #pragma omp parallel shared(board) private(i, j)
    {
        #pragma omp for
        for (i = 1; i < m - 1; i++) {
            board[i][0] = 100.0;
    ...
    #pragma omp parallel shared (average, board) private (i, j)
    {
        #pragma omp for
        for (int i = 1; i < m - 1; i++) {
            for (int j = 1; j < n - 1; j++) {
...
int main(int argc, char** argv)
{
    ...
    #pragma omp parallel shared(myboard_new) private (i, j)
    {
        if (myid == 0) {
	...
	while (iters < MAXITERS)
	{
        ...
        #pragma omp parallel shared(myboard_new, myboard, myrow_bot, myrow_top) private(i, j)
        {
            #pragma omp for
            for (i = 0; i < myrows; i++){
                for (j = 1; j < (N-1); j++){
