...
int main(int argc, char * argv[]) {
    ...
    while (iters < MAX_ITERS) {
        ...
        */
        if (processId > 0) {
            MPI_Sendrecv(&localTable[0][0], 1, sendColumn, (processId - 1), 0,
                        localLeftColumn, M, MPI_DOUBLE, (processId - 1), 1,
                        MPI_COMM_WORLD, &status);
        }
        if (processId < processNum - 1) {
            MPI_Sendrecv(&localTable[0][columnsNum - 1], 1, sendColumn, (processId + 1), 1,
                        localRightColumn, M, MPI_DOUBLE, (processId + 1), 0,
                        MPI_COMM_WORLD, &status);
        }
        /*
    ...
    if (processId == 0) {
        ...
        std::cout << "MPI Columns Blocking. Execution time: " << runningTime << " ms." << std::endl;
